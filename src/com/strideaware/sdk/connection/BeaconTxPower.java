  ...
  implements BluetoothService
{
	...
	public void processGattServices(List<BluetoothGattService> services)
	{
		for (BluetoothGattService service : services)
			if (StrideAwareUuid.BEACON_TX_POWER_SERVICE.equals(service.getUuid())) {
				if (service.getCharacteristic(StrideAwareUuid.BEACON_TX_POWER_CHAR) != null)
				{
					this.characteristics.put(StrideAwareUuid.BEACON_TX_POWER_CHAR, service.getCharacteristic(StrideAwareUuid.BEACON_TX_POWER_CHAR));	
				}
			}
	}
	...
	public Integer getBeaconTxPower() {
		int temp = this.characteristics.containsKey(StrideAwareUuid.BEACON_TX_POWER_CHAR) ? Integer.valueOf(getUnsignedByte(((BluetoothGattCharacteristic)this.characteristics.get(StrideAwareUuid.BEACON_TX_POWER_CHAR)).getValue())) : null;
		int txPower;
		if (mCurrentIsStridAwareNewBeacon)
		{
			switch(temp)
