...
import com.strideaware.sdk.BLEDevice;
import com.strideaware.sdk.Beacon;
import com.strideaware.sdk.Utils;
import com.strideaware.sdk.internal.HashCode;
import com.strideaware.sdk.utils.L;
...
//	private final Runnable timeoutHandler;
	...
	private boolean mStartWritePass = false;//�Ƿ���StridAware�°�Ĺ̼�
	public boolean mCurrentIsStridAwareNewBeacon = false;//�Ƿ���StridAware�°�Ĺ̼�
	...
	private final StrideAwareService mBeaconService;
	...
	{
		mCurrentIsStridAwareNewBeacon = false;
...
//		this.timeoutHandler = createTimeoutHandler();
		...
		this.mBeaconService = new StrideAwareService();
		...
		this.uuidToService.put(StrideAwareUuid.BEACON_BATTERY_LIFE, mBatteryService);
		this.uuidToService.put(StrideAwareUuid.StridAware_BEACON_SERVICE, mBeaconService);
		this.uuidToService.put(StrideAwareUuid.BEACON_ALERT, mAlertService);
		this.uuidToService.put(StrideAwareUuid.BEACON_NAME, mNameService);		
		this.uuidToService.put(StrideAwareUuid.BEACON_STATE_SERVICE, mBeaconStateService);
		this.uuidToService.put(StrideAwareUuid.BEACON_AUDIO_STATE_SERVICE, mBeaconAudioStateService);
		this.uuidToService.put(StrideAwareUuid.BEACON_TX_POWER_SERVICE, mBeaconTxPowerService);
	}
...
//		this.timeoutHandler = createTimeoutHandler();
	...
	public void connectBeaconWithPassword(String password)
	{
		...
		this.mCurrentIsStridAwareNewBeacon = false;
		 BeaconNameService.mCurrentIsStridAwareNewBeacon = false;
		 BeaconTxPower.mCurrentIsStridAwareNewBeacon = false;
		 BeaconAudioService.mCurrentIsStridAwareNewBeacon = false;
		 BeaconStateService.mCurrentIsStridAwareNewBeacon = false;
		 StrideAwareService.mCurrentIsStridAwareNewBeacon = false;
...
//		return (connectionState == 2) && (this.didReadCharacteristics);
	...
	{
		if ((!isConnected()) || (!this.mAlertService.hasCharacteristic(StrideAwareUuid.BEACON_ALERT_CHAR))) 
		{
			L.w("Not connected to beacon. Discarding changing proximity UUID.");
//			writeCallback.onError();
			return;
		}
		BluetoothGattCharacteristic AlertChar = this.mAlertService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_ALERT_CHAR, null);
	...
	{
		if ((!isConnected()) || (!this.mBeaconService.hasCharacteristic(StrideAwareUuid.BEACON_KEEP_CONNECT_CHAR))) 
		{
			L.w("Not connected to beacon. Discarding changing proximity UUID.");
			writeCallback.onError();
			return;
		}
		BluetoothGattCharacteristic uuidChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_KEEP_CONNECT_CHAR, writeCallback);
...
//			Log.i("NRF  TEST121", "NRF  TIд��");
	...
	{
		if ((!isConnected()) || (!this.mBeaconService.hasCharacteristic(StrideAwareUuid.BEACON_UUID_CHAR))) {
			L.w("Not connected to beacon. Discarding changing proximity UUID.");
			writeCallback.onError();
			return;
		}
		...
		{
			PasswordChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_FFF8, writeCallback);
			PasswordChar.setValue(NewPasswordBytes);
		}
		else
		{
			PasswordChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_FFF6, writeCallback);
		...
		if (!mCurrentIsStridAwareNewBeacon)
		{
			new Handler().postDelayed(new Runnable(){    
			    public void run() {    
			    	//execute the task    
//			    	BeaconConnection.this.toFetch.add(BeaconConnection.this.mBeaconService.getAvailableCharacteristic(StridAwareUuid.CHANGE_PASSWORD_CHAR));
	...
	public void writeProximityUuid(String proximityUuid, WriteCallback writeCallback)
	{
		...
		final BluetoothGattCharacteristic uuidChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_UUID_CHAR, writeCallback);
		...
		if (mCurrentIsStridAwareNewBeacon)
		{
			...
			new Handler().postDelayed(new Runnable(){    
			    public void run() {    
			    //execute the task    
//			    	BeaconConnection.this.toFetch.add(BeaconConnection.this.mBeaconService.getAvailableCharacteristic(StridAwareUuid.BEACON_UUID_CHAR));
	...
	{
		if ((!isConnected()) || (!this.mBeaconService.hasCharacteristic(StrideAwareUuid.POWER_CHAR))) {
	    ...
	    final BluetoothGattCharacteristic powerChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.POWER_CHAR, writeCallback);
	...
	public void writeMajor(int major, WriteCallback writeCallback)
	{
		...
		final BluetoothGattCharacteristic majorChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.MAJOR_CHAR, writeCallback);
	...
	public void writeMinor(int minor, WriteCallback writeCallback)
	{
		...
		final BluetoothGattCharacteristic minorChar = this.mBeaconService.beforeCharacteristicWrite(StrideAwareUuid.MINOR_CHAR, writeCallback);
	...
	public void writeBeaconState(int state, WriteCallback writeCallback)
	{
		...
		switch (state)
		{
			case StrideAwareDefine.StridAware_BEACON_STATE_ENABLE:
				arrayOfByte[0] = 2;
				break;
			case StrideAwareDefine.StridAware_BEACON_STATE_DISABLE:
				arrayOfByte[0] = 1;
				break;
			default:
				arrayOfByte[0] = 1;
		...
		final BluetoothGattCharacteristic Char = this.mBeaconStateService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_STATE_CHAR, writeCallback);
	...
	public void writeBeaconAudioState(int state, WriteCallback writeCallback)
	{
		...
		switch (state)
		{
			case StrideAwareDefine.BEACON_AUDIO_STATE_ENABLE:
				arrayOfByte[0] = 1;
				break;
			case StrideAwareDefine.BEACON_AUDIO_STATE_ENABLE_WHEN_START:
				arrayOfByte[0] = 2;
				break;
			case StrideAwareDefine.BEACON_AUDIO_STATE_ENABLE_WHEN_TAP:
				arrayOfByte[0] = 3;
				break;
			case StrideAwareDefine.BEACON_AUDIO_STATE_DISABLE:
				arrayOfByte[0] = 4;
				break;
			default:
				arrayOfByte[0] = 1;
		...
		final BluetoothGattCharacteristic Char = this.mBeaconAudioStateService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR, writeCallback);
		Char.setValue(arrayOfByte);
		this.bluetoothGatt.writeCharacteristic(Char);
	}
	 ...
	 * Allowed values: see StridAwareDefine	
	...
	public void writeBeaconTxPower(int state, WriteCallback writeCallback)
	{
		...
		if (mCurrentIsStridAwareNewBeacon)
		{
			switch (state)
			{
				case StrideAwareDefine.StridAware_TX_POWER_4_DBM:
					arrayOfByte[0] = 1;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_0_DBM:
					arrayOfByte[0] = 2;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_4_DBM:
					arrayOfByte[0] = 3;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_8_DBM:
					arrayOfByte[0] = 4;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_12_DBM:
					arrayOfByte[0] = 5;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_16_DBM:
					arrayOfByte[0] = 6;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_20_DBM:
					arrayOfByte[0] = 7;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_30_DBM:
					arrayOfByte[0] = 8;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_40_DBM:
					arrayOfByte[0] = 9;
					break;					
				default:
					arrayOfByte[0] = 1;
		...
		else
		{
			switch (state)
			{
				case StrideAwareDefine.StridAware_TX_POWER_0_DBM:
					arrayOfByte[0] = 0;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_6_DBM:
					arrayOfByte[0] = 1;
					break;
				case StrideAwareDefine.StridAware_TX_POWER_MINUS_23_DBM:
					arrayOfByte[0] = 2;
					break;
				default:
					arrayOfByte[0] = 0;
		...
		final BluetoothGattCharacteristic Char = this.mBeaconTxPowerService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_TX_POWER_CHAR, writeCallback);
	 ...
	 * @param name the name of the beacon,like "StridAware"
	 * @param writeCallback Callback to be invoked when write is completed.
	 */
	public void writeBeaconName(String name, WriteCallback writeCallback)
	{
		...
		if (mCurrentIsStridAwareNewBeacon)
		{
			Char = this.mNameService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_NEW_NAME_CHAR, writeCallback);			
		}
		else
		{
			Char = this.mNameService.beforeCharacteristicWrite(StrideAwareUuid.BEACON_NAME_CHAR, writeCallback);	
		}
		...
		return new BluetoothGattCallback()
		{
			...
			public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status)
			{
				if (status == 0) {
					...
					if (StrideAwareUuid.BEACON_KEEP_CONNECT_CHAR.equals(characteristic.getUuid()))
					{
						//��ȡ����ֵ
						//keep connect
...
//					BeaconConnection.this.startAuthentication(gatt);
					 ...
					 for (BluetoothGattService service : gatt.getServices())
					 {
						 if (StrideAwareUuid.BEACON_STATE_SERVICE.equals(service.getUuid())) {
							 mCurrentIsStridAwareNewBeacon = true;//Nordic Beacon
							 BeaconNameService.mCurrentIsStridAwareNewBeacon = true;
							 BeaconTxPower.mCurrentIsStridAwareNewBeacon = true;
							 BeaconAudioService.mCurrentIsStridAwareNewBeacon = true;
							 BeaconStateService.mCurrentIsStridAwareNewBeacon = true;
							 StrideAwareService.mCurrentIsStridAwareNewBeacon = true;
							 
						 }
						 else if (StrideAwareUuid.StridAware_BEACON_SERVICE.equals(service.getUuid()))
						 {
							 //��ȡ����ֵ
								BeaconConnection.this.handler.postDelayed(new Runnable() 
								{
