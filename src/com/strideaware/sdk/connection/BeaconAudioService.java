  ...
  implements BluetoothService
{
	...
	public void processGattServices(List<BluetoothGattService> services)
	{
		for (BluetoothGattService service : services)
			if (StrideAwareUuid.BEACON_AUDIO_STATE_SERVICE.equals(service.getUuid())) {
				
				if (service.getCharacteristic(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR) != null)
				{
					this.characteristics.put(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR, service.getCharacteristic(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR));	
				}
			}
	}
	...
	public Integer getBeaconAudioState() {
		...
		{
			return this.characteristics.containsKey(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR) ? Integer.valueOf(getUnsignedByte(((BluetoothGattCharacteristic)this.characteristics.get(StrideAwareUuid.BEACON_AUDIO_STATE_CHAR)).getValue())) : null;	
		}
		return 0;
