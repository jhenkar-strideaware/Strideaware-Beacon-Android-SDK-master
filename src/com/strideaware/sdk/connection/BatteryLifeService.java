  ...
  implements BluetoothService
{
	...
	public void processGattServices(List<BluetoothGattService> services)
	{
		for (BluetoothGattService service : services)
			if (StrideAwareUuid.BEACON_BATTERY_LIFE.equals(service.getUuid())) {
				this.characteristics.put(StrideAwareUuid.BEACON_BATTERY_LIFE_CHAR, service.getCharacteristic(StrideAwareUuid.BEACON_BATTERY_LIFE_CHAR));
			}
	}
	
	public Integer getBatteryPercent() {
		return this.characteristics.containsKey(StrideAwareUuid.BEACON_BATTERY_LIFE_CHAR) ? Integer.valueOf(getUnsignedByte(((BluetoothGattCharacteristic)this.characteristics.get(StrideAwareUuid.BEACON_BATTERY_LIFE_CHAR)).getValue())) : null;
	}
